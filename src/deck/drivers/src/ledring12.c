...
#define LEDRING_TIME_MEM_SIZE 1024
#endif

typedef struct __attribute__((packed)) timing {
  uint8_t duration[2];      // How long this color should be show in parts of 1/100s. So 100 will show the color 1s, before going to the next color.
...
#define LEDRING_TIME_MEM_SEC 1000 / 100
...
{
  return current.duration[0] == 0  && current.duration[1] == 0 
      && current.color[0] == 0 && current.color[1] == 0;
}

static uint16_t getDuration(ledtiming timing)
{
  uint16_t result = timing.duration[0];
  return ((result << 8) | timing.duration[1]);
}
...
static void timeMemEffect(uint8_t outputBuffer[][3], bool reset)
{
  ...
  uint16_t duration = getDuration(current);
  ...
  while (timeEffectTime + LEDRING_TIME_MEM_SEC * duration < time) {
    ...
    // Goto next effect
    timeEffectTime += LEDRING_TIME_MEM_SEC * duration;
    timeEffectI++;
  ...
  float percent = 1.0 * (time - timeEffectTime) / (duration * LEDRING_TIME_MEM_SEC);
  for (int i = 0; i < NBR_LEDS; i++)
    for (int j = 0; j < 3; j++)
  ...
  for (int i = 0; i < NBR_LEDS; i++)
    for (int j = 0; j < 3; j++)
      outputBuffer[i][j] = currentBuffer[i][j];
}
